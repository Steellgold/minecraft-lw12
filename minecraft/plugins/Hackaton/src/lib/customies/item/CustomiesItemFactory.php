...
final class CustomiesItemFactory {
	 ...
	 */
	public function registerItem(string $className, string $identifier): void {
		if($className !== Item::class) {
			Utils::testValidInstance($className, Item::class);
		}

		$itemId = ItemTypeIds::newId();
		$item = new $className(new ItemIdentifier($itemId), str_replace("wapy:", "", $identifier));
        $this->toItem($identifier, $itemId, $item);
    }
     ...
     * @phpstan-param class-string $className
     */
    public function lotoRegisterItem(string $className, string $identifier, int $number = 1, bool $selected = false, bool $lucky = false, bool $other = false) : void {
        if($className !== Item::class) {
            Utils::testValidInstance($className, Item::class);
        }

        $itemId = ItemTypeIds::newId();
        $item = new $className(new ItemIdentifier($itemId), str_replace("wapy:", "", $identifier), $number, $selected, $lucky, $other);
        $this->toItem($identifier, $itemId, $item);
    }
     ...
     * @return void
     */
    public function toItem(string $identifier, int $itemId, mixed $item): void {
        $this->registerCustomItemMapping($identifier, $itemId);
